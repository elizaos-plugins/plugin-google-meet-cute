...
export class GoogleMeetService extends Service {
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    ...
    this.meetConfig = {
      ...
      DEFAULT_MEETING_NAME:
        runtime.getSetting("DEFAULT_MEETING_NAME") || "ElizaOS Bot",
      TRANSCRIPTION_LANGUAGE:
        runtime.getSetting("TRANSCRIPTION_LANGUAGE") || "en",
      REPORT_OUTPUT_DIR:
        runtime.getSetting("REPORT_OUTPUT_DIR") || "./meeting-reports",
      ENABLE_REAL_TIME_TRANSCRIPTION:
        runtime.getSetting("ENABLE_REAL_TIME_TRANSCRIPTION") === "true",
      AUDIO_CHUNK_DURATION_MS: parseInt(
        runtime.getSetting("AUDIO_CHUNK_DURATION_MS") || "30000",
      ),
    };
  ...
  async joinMeeting(
    ...
    if (this.state.currentMeeting) {
      throw new Error(
        "Already in a meeting. Please leave the current meeting first.",
      );
    }
  ...
  private async handleGoogleLogin(page: any): Promise<void> {
    ...
    if (this.meetConfig.GOOGLE_MEET_PASSWORD) {
      ...
      await page.type(
        'input[type="password"]',
        this.meetConfig.GOOGLE_MEET_PASSWORD,
      );
